...
function CreateOrder() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare order data - CRITICAL: Use correct field names
      const orderData = {
        ...
        <form onSubmit={handleSubmit}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (newCustomer.name) {
                          ...
                          setFormData(prev => ({ ...prev, customerId: Date.now() }));
                          setShowNewCustomerForm(false);
                        } else {
              ...
              >
                ...
                {products.map(product => (
                  <option key={product.id} value={product.id}>
                    {product.name} - ${product.price?.toFixed(2) || '0.00'} ({product.stock || 0} in stock)
                  </option>
                ))}
              </select>
                ...
                <div className="selected-info">
                  <div className="product-details-grid">
                    ...
                    <div>
                      ...
                      <p><strong>Stock:</strong> 
                        <span className={isLowStock(selectedProduct) ? "stock-low" : "stock-ok"}>
                  ...
                  {isLowStock(selectedProduct) && (
                    <div className="warning-message">
                      ⚠️ <strong>Low Stock Alert:</strong> Only {selectedProduct.stock} left in stock
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="form-section">
            ...
            <div className="order-summary">
              <div className="summary-row">
                <span>Customer:</span>
                <span>{selectedCustomer ? selectedCustomer.name : (newCustomer.name || "Not selected")}</span>
              </div>
              <div className="summary-row">
                <span>Product:</span>
                <span>{selectedProduct ? selectedProduct.name : "Not selected"}</span>
              </div>
              <div className="summary-row">
                <span>Unit Price:</span>
                <span>${selectedProduct ? selectedProduct.price?.toFixed(2) || '0.00' : "0.00"}</span>
              </div>
              <div className="summary-row">
                <span>Quantity:</span>
          ...
          <div className="form-actions">
            ...
            <button
              ...
              disabled={loading || (!formData.customerId && !newCustomer.name) || !formData.productId}
            >
